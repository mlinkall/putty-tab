...
static
char* WideToAnsi(const wchar_t* wstr) 
{
...
{
	int tabIndex = tabGetIndexByHwnd(hwnd);
	TCITEM tci = {0};
	tci.mask = TCIF_TEXT;

	char *_text = WideToAnsi(text);
    if (unicode_window) {
        SetWindowTextW(hwnd, text);
    	tci.pszText = _text;
        TabCtrl_SetItem(hTabCtrl, tabIndex, &tci);
    } else {
...
static
void terminal_create(int tabIndex, WinGuiSeat *wgs)
{
	int guess_width;
	int guess_height;
...

    {
        ...
        //DialogAdd(hinst, guess_width, guess_height);
        {
        RECT clientRect;
        GetClientRect(hDialog, &clientRect);
        //GetWindowRect(hDialog, &clientRect);
        int clientWidth = clientRect.right - clientRect.left;
        int clientHeight = clientRect.bottom - clientRect.top;


    	PUTTY_LOG(PUTTY_LOG_DEBUG, "client:%d,%d, calc:%d,%d",
    			clientWidth, clientHeight, guess_width, guess_height);

        guess_width = clientWidth;
		guess_height = clientHeight;
        }
        ...
        wgs->term_hwnd = CreateWindowExW(
            ...
            winmode, 0, 0, //nCaptionHeight + 2,
            guess_width, guess_height/* - nCaptionHeight -2*/, hDialog, NULL, hinst, NULL);

        hPages[tabIndex] = wgs->term_hwnd;
#endif
     ...
     */
    ShowWindow(wgs->term_hwnd, SW_SHOW);
...
{
	PUTTY_LOG(PUTTY_LOG_DEBUG, "hwnd:%d, message:%u, wParam:%d, lParam:%d",
	...
	case WM_INITDIALOG:
		{
			...
			int i;
			for (i = 0; i < tabPageCount(); i++)
			{
				MoveWindow(hPages[i], 0, nCaptionHeight, width, height, TRUE);
			}
			return 0;
		}
		break;
	...
	case WM_SIZE:
		{
			...
			for (i = 0; i < tabPageCount(); i++)
			{
				PUTTY_LOG(PUTTY_LOG_DEBUG, "WM_SIZE id:%d", i);
				MoveWindow(hPages[i], 0, nCaptionHeight + 2, width, height - nCaptionHeight - 2, TRUE);
			}
			return 0;
		}
		break;
	case WM_NOTIFY:
		{
			...
			PUTTY_LOG(PUTTY_LOG_DEBUG, "WM_NOTIFY msg:%u,code:%d",
					msg, pnmh->code);
			switch(pnmh->code)
			{
			case TCN_SELCHANGE:
				{
					...
					if (tabIndex < MAX_WINDOW)
					{
						ShowWindow(hPages[tabIndex], SW_SHOW);

						tabSetActive(tabIndex);

						if (tabGet(tabIndex) == NULL)
						{
						    WinGuiSeat *wgs;
							tabAdd(&wgs);
			...
			case TCN_SELCHANGING:
				{
					int selected_tab = TabCtrl_GetCurSel(pnmh->hwndFrom);
					if (selected_tab < MAX_WINDOW)
					{
						ShowWindow(hPages[selected_tab], SW_HIDE);
					}
					return 0;
				}
				break;
	...
	default:
		if (tabGetActive() != NULL)
		{
			...
			case WM_SYSKEYUP:
				PUTTY_LOG(PUTTY_LOG_DEBUG, "hwnd:%d, message:%u, wParam:%d, lParam:%d",
				    		hwnd, msg, wParam, lParam);
				SendMessage(tabGetActive()->term_hwnd, msg, wParam, lParam);
				return 0;
			default:
				break;
			}
...
int WINAPI WinMain(HINSTANCE inst, HINSTANCE prev, LPSTR cmdline, int show)
{
...

    {
        ...
        DialogAdd(inst, guess_width, guess_height);
        RECT clientRect;
        GetClientRect(hDialog, &clientRect);
        int clientWidth = clientRect.right - clientRect.left;
        int clientHeight = clientRect.bottom - clientRect.top;
		...
		PUTTY_LOG(PUTTY_LOG_DEBUG, "client:%d,%d, calc:%d,%d, wgs:%p",
        		clientWidth, clientHeight, guess_width, guess_height, wgs);
        ...
        wgs->term_hwnd = CreateWindowExW(
            exwinmode, terminal_window_class_w(), uappname,
            winmode, 0, 2,
            guess_width, guess_height - nCaptionHeight -2, hDialog, NULL, inst, NULL);

        hPages[0] = wgs->term_hwnd;
        tabSetActive(0);
#endif
    ...
    //RECT wr;
    //GetWindowRect(wgs->term_hwnd, &wr);
    //InvalidateRect(wgs->term_hwnd, &wr, 1);
...
#define TO_CHR_Y(y) ((((y)<0 ? (y)-wgs->font_height+1 :                 \
      ...
      case WM_SYSKEYUP:
         ...
         */
        {
            ...
            if (wParam == VK_PROCESSKEY || /* IME PROCESS key */
                ...
                PUTTY_LOG(PUTTY_LOG_DEBUG, "key:%d[%02x,%02x,%02x,%02x,%02x,%02x,%02x,%02x,%02x,%02x,",
                		len,
						(char)wParam,buf[1],buf[2],buf[3],buf[4],
      ...
      case WM_SYSCHAR:
         ...
         * we're ready to cope.
         */
          PUTTY_LOG(PUTTY_LOG_DEBUG, "key_key:%d[%c],",
          		message,(unsigned char)wParam);
        if (unicode_window) {
            wchar_t c = wParam;
